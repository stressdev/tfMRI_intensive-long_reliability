...
processVoxel <- function(v) {
  ...
  model_formula <- BRAIN ~ 1 + TIMECENTER
  ...
  e_sandwich <- try(time.happy_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(time.happy))) {
    coefs <- coef(summary(time.happy))
    ...
    std_coefs <- coef(beta(time.happy))
  ...
  if(!(is.null(time.happy_sw))){
    coefs_sw <- time.happy_sw
