...
processVoxel <- function(v) {
  ...
  e_sandwich <- try(sleepdur.fear_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(sleepdur.fear))) {
    ...
    std_coefs <- coef(beta(sleepdur.fear))
  ...
  if(!(is.null(sleepdur.fear_sw))){
    coefs_sw <- sleepdur.fear_sw
