...
processVoxel <- function(v) {
  ...
  e_sandwich <- try(chsev.happy_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(chsev.happy))) {
    coefs <- coef(summary(chsev.happy))
    ...
    std_coefs <- coef(beta(chsev.happy))
  ...
  if(!(is.null(chsev.happy_sw))){
    coefs_sw <- chsev.happy_sw
