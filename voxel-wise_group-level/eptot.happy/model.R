...
processVoxel <- function(v) {
  ...
  e_sandwich <- try(eptot.happy_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(eptot.happy))) {
    coefs <- coef(summary(eptot.happy))
    ...
    std_coefs <- coef(beta(eptot.happy))
  ...
  if(!(is.null(eptot.happy_sw))){
    coefs_sw <- eptot.happy_sw
