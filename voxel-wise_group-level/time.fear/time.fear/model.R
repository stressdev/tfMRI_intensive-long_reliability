...
processVoxel <- function(v) {
  ...
  e_sandwich <- try(time.fear_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(time.fear))) {
    coefs <- coef(summary(time.fear))
  ...
  if(!(is.null(time.fear_sw))){
    coefs_sw <- time.fear_sw
